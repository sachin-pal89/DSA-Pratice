...
// For Taking input in linked list
Node* TakeInput()
{
    int data;
    cin>> data;
...
// For printing elements of the Linked list
...
// For deleting a specific node from linked list by its index
Node* DeleteNode(Node* head, int index)
{
    ...
    // If Element to be deleted is head only
    if(index == 0)
    {
        Node* temp = head;
        head = head->next;
    ...
    // If The element to be deleted is next element then perform this operation
    ...
    // Since the return node would be joined with our head
